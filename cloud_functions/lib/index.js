...
exports.cleanupOldNotifications = exports.onEmployeeCheckIn = exports.processFcmSendQueue = exports.processScheduledNotifications = void 0;
...
exports.processScheduledNotifications = functions
    ...
    functions.logger.info(`[ScheduledNotifs] Processing ${snap.docs.length} notifications`);
    const batch = db.batch();
    const sends = [];
    for (const doc of snap.docs) {
        const data = doc.data();
        let token = (_a = data.fcm_token) !== null && _a !== void 0 ? _a : "";
        const title = (_b = data.title) !== null && _b !== void 0 ? _b : "Abhishek HRMS";
        const body = (_c = data.body) !== null && _c !== void 0 ? _c : "";
        const type = (_d = data.type) !== null && _d !== void 0 ? _d : "custom";
        const employeeId = (_e = data.employee_id) !== null && _e !== void 0 ? _e : "";
        ...
        if (!token && employeeId) {
            functions.logger.info(`[ScheduledNotifs] Doc ${doc.id} has no fcm_token — fetching from employee doc ${employeeId}`);
            try {
                const empSnap = await db.collection("employees").doc(employeeId).get();
                if (empSnap.exists) {
                    token = (_g = (_f = empSnap.data()) === null || _f === void 0 ? void 0 : _f.fcm_token) !== null && _g !== void 0 ? _g : "";
                    if (token) {
                        functions.logger.info(`[ScheduledNotifs] ✅ Found token from employee doc for ${employeeId}`);
        ...
        if (!token) {
            functions.logger.warn(`[ScheduledNotifs] Doc ${doc.id} — no token available for ${employeeId}, skipping`);
        ...
        // Queue the send
        sends.push(messaging.send(message).then((msgId) => {
            functions.logger.info(`[ScheduledNotifs] ✅ Sent ${type} to ${employeeId} — msgId: ${msgId}`);
            return msgId;
        }));
...
// ─────────────────────────────────────────────────────────────────────────────
exports.processFcmSendQueue = functions
    .region(REGION)
    .firestore
    ...
    if (data.processed === true)
        return;
    const tokens = (_a = data.tokens) !== null && _a !== void 0 ? _a : [];
    const title = (_b = data.title) !== null && _b !== void 0 ? _b : "Abhishek HRMS";
    const body = (_c = data.body) !== null && _c !== void 0 ? _c : "";
    const priority = (_d = data.priority) !== null && _d !== void 0 ? _d : "normal";
    const targetType = (_e = data.target_type) !== null && _e !== void 0 ? _e : "global";
    if (tokens.length === 0) {
        await snap.ref.update({ processed: true, error: "no_tokens" });
        functions.logger.warn(`[FcmQueue] ${snap.id} — no tokens, skipped`);
        return;
    }
    functions.logger.info(`[FcmQueue] Sending to ${tokens.length} device(s) — target: ${targetType}`);
    ...
    for (let i = 0; i < tokens.length; i += 500) {
        ...
        functions.logger.info(`[FcmQueue] Chunk sent — success: ${successCount}, fail: ${failCount}`);
        // Auto-clean stale tokens from employee docs
        const staleCleaner = [];
        response.responses.forEach((resp, idx) => {
            var _a, _b;
            if (!resp.success) {
                const errCode = (_b = (_a = resp.error) === null || _a === void 0 ? void 0 : _a.code) !== null && _b !== void 0 ? _b : "";
                const staleErrors = [
    ...
    functions.logger.info(`[FcmQueue] ✅ Done — ${totalSuccess}/${tokens.length} delivered`);
});
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
exports.onEmployeeCheckIn = functions
    .region(REGION)
    .firestore
    ...
    const att = snap.data();
    const employeeId = (_b = (_a = att.employee_id) !== null && _a !== void 0 ? _a : att.employeeId) !== null && _b !== void 0 ? _b : "";
    if (!employeeId)
        return;
    // Fetch the employee record to get FCM token + shift times
    const empSnap = await db.collection(COL_EMPLOYEES).doc(employeeId).get();
    if (!empSnap.exists) {
        functions.logger.warn(`[CheckIn] Employee ${employeeId} not found in Firestore`);
        return;
    }
    const emp = empSnap.data();
    const fcmToken = (_c = emp.fcm_token) !== null && _c !== void 0 ? _c : "";
    const shiftEnd = (_e = (_d = emp.shift_end_time) !== null && _d !== void 0 ? _d : emp.shiftEndTime) !== null && _e !== void 0 ? _e : "";
    const shiftStart = (_g = (_f = emp.shift_start_time) !== null && _f !== void 0 ? _f : emp.shiftStartTime) !== null && _g !== void 0 ? _g : "";
    const shiftType = (_j = (_h = emp.shift_type) !== null && _h !== void 0 ? _h : emp.shiftType) !== null && _j !== void 0 ? _j : "Day Shift";
    ...
    if (!fcmToken) {
        functions.logger.info(`[CheckIn] ${employeeId} has no FCM token yet — scheduling anyway, ` +
    ...
    if (endDt) {
        ...
        if (reminderAt > now) {
            scheduled.push(_upsertScheduledNotif({
                employeeId,
                fcmToken,
            ...
            functions.logger.info(`[CheckIn] Logout reminder for ${employeeId} at ${reminderAt.toISOString()}`);
        }
    }
    ...
    if (startDt) {
        ...
        const shiftReminderAt = new Date(tomorrowStart.getTime() - 15 * 60 * 1000); // -15 mins
        if (shiftReminderAt > now) {
            ...
            functions.logger.info(`[CheckIn] Shift reminder for ${employeeId} at ${shiftReminderAt.toISOString()}`);
        }
    }
...
exports.cleanupOldNotifications = functions
    ...
    .timeZone("Asia/Kolkata")
    .onRun(async () => {
    const cutoff = admin.firestore.Timestamp.fromDate(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) // 7 days ago
    );
    const snap = await db
        .collection(COL_SCHEDULED)
    ...
    await batch.commit();
    functions.logger.info(`[Cleanup] ✅ Deleted ${snap.docs.length} old notifications`);
});
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
function _parseShiftTime(timeStr, baseDate) {
    if (!timeStr)
        return null;
    try {
...
// ─────────────────────────────────────────────────────────────────────────────
async function _clearStaleToken(token) {
    try {
        ...
        functions.logger.info(`[StaleToken] Cleared dead token from ${snap.docs[0].id}`);
    }
    catch (e) {
